...
function createApp({ installService, gameMetadataService, log, healthInfo, config }) {
  const app = express();
  app.use(express.json());

  app.use(express.static(path.join(__dirname, "..", "ui", "dist", "ui", "browser")));

  app.use("/api", createApiRouter({ installService, gameMetadataService, log, healthInfo, config }));
